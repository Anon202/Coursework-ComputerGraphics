...
namespace graphics_framework
{
	...
	*/
	...
	bool geometry::add_buffer(const std::vector<float> &buffer, GLuint index, GLenum buffer_type)
	{
		// Check that index is viable
		assert(index < 16);
		// Check that buffer is not empty
		...
		*/
		...
		glBindBuffer(GL_ARRAY_BUFFER, id);
		// Set the buffer data
		glBufferData(GL_ARRAY_BUFFER, buffer.size() * sizeof(float), &buffer[0], buffer_type);
		// Set the vertex pointer and enable
		glVertexAttribPointer(index, 1, GL_FLOAT, GL_FALSE, 0, 0); // ONE FLOAT
		glEnableVertexAttribArray(index);
		// Check for OpenGL error
		if (CHECK_GL_ERROR)
		{
